...
namespace RequestTrackerFEAPP
{
    internal class Program
    {
        ...
        private readonly ISolutionBL solutionBL;
        ...
        {
            this.employeeLoginBL = employeeLoginBL;
            this.requestBL = requestBL;
            this.solutionBL = solutionBL;
        ...
        static async Task Main(string[] args)
        {
            Program program = new Program(
                new EmployeeLoginBL(new EmployeeRepository(new RequestTrackerContext())),
                new RequestBL(new RequestRepository(new RequestTrackerContext())),
                new SolutionBL(new RequestSolutionRepository(new RequestTrackerContext())),
                new FeedbackBL(new SolutionFeedbackRepository(new RequestTrackerContext()))
            );
            bool exit = false;
        ...
        async Task AddRequest()
        {
            ...
            Request request = new Request
            {
                ...
                RequestRaisedBy = CurrentLoggedInEmployee?.Id?? 0,
        ...
        {
            var requestDetails = await requestBL.GetRequestwithID(CurrentLoggedInEmployee.Id);
        ...
        async Task ViewSolutions()
        {
            ...
            if (request.RequestRaisedBy != CurrentLoggedInEmployee.Id)
            {
                Console.WriteLine("You are not authorized to view solutions for this request.");
                return;
            }
            ...
            foreach (var sol in solution)
            {
                ...
                {
                    Console.WriteLine($"Solution ID: {sol.SolutionId}, Description: {sol.SolutionDescription}");
                }
            }
        ...
        async Task RespondToSolution()
        {
            ...
            var solution = await solutionBL.GetSolution(solutionId);
            if (solution == null)
            {
            ...
            if(solution.RequestRaiserComment != null)
            {
                Console.WriteLine("Response already submitted.");
                return;
            }

            solution.RequestRaiserComment = response;
            await solutionBL.UpdateSolution(solution);
        ...
        async Task ProvideSolution()
        {
            ...
            int requestNumber = Convert.ToInt32(Console.ReadLine());
            var res = await requestBL.GetRequest(requestNumber);
        ...
        {
            var allSolutions = await solutionBL.GetAllSolutions();
